...
pub(crate) enum D1DatabaseRecoveryState {
    Present {
        uuid: String,
        name: String,
...
#[derive(Debug, Clone)]
pub(crate) struct D1DatabaseRecoveryRead {
    pub(crate) state: D1DatabaseRecoveryState,
    pub(crate) response_body_sha256: String,
    pub(crate) response_body_size_bytes: usize,
    pub(crate) lifecycle: D1MigrationReconciliationReadLifecycle,
}
...
#[derive(Debug, Clone)]
pub(crate) struct D1DatabaseRecoveryReadError {
    pub(crate) error: AdapterErrorPayload,
    pub(crate) response_body_sha256: Option<String>,
    pub(crate) response_body_size_bytes: Option<usize>,
    pub(crate) lifecycle: D1MigrationReconciliationReadLifecycle,
...
/// the complete duplicate-free Cloudflare envelope product. Keep these local:
/// the per-operation result shapes and ResponseInfo policy are provider-specific,
/// while the resulting lifecycle classification uses the toolkit's shared
/// transport-neutral `MutationApplyStatus`.
#[derive(Debug, Deserialize)]
#[serde(deny_unknown_fields)]
struct StrictD1DatabaseRenameEnvelope {
    success: bool,
    result: Value,
    errors: Value,
...
#[derive(Debug, Deserialize)]
#[serde(deny_unknown_fields)]
struct StrictD1DatabaseDeleteEnvelope {
    success: bool,
    #[serde(default)]
    result: Option<Value>,
    errors: Value,
...
#[derive(Debug, Deserialize)]
#[serde(deny_unknown_fields)]
struct StrictD1DatabaseRecoveryEnvelope {
    success: bool,
    result: Option<Value>,
    errors: Value,
    messages: Value,
...
#[derive(Debug, Deserialize)]
#[serde(deny_unknown_fields)]
struct StrictD1DatabaseRecoveryResult {
    uuid: String,
    name: String,
    #[serde(default)]
...
struct StrictD1DatabaseMutationResponseInfo {
    ...
    #[serde(
        default,
        deserialize_with = "deserialize_present_non_null_option",
...
enum D1DatabaseMutationEnvelopeContract {
    Rename,
    Delete,
}
...
impl CloudflareClient {
    ...
    /// Read one exact D1 database through the bounded no-redirect client used
    /// for reconciliation. Both a present object and a strict authenticated
    /// 404 absence envelope retain complete response-body evidence.
    pub(crate) async fn read_d1_database_for_recovery(
        &self,
        account_id: &str,
        database_id: &str,
    ) -> Result<D1DatabaseRecoveryRead, D1DatabaseRecoveryReadError> {
        let pre_dispatch = |error: AdapterError| D1DatabaseRecoveryReadError {
            error: error.payload(),
        ...
        let request = self
            ...
            .map_err(|error| {
                pre_dispatch(AdapterError::new(
                    ...
                    format!("Cloudflare D1 recovery read request build failed: {error}"),
                    "Correct the recovery-read configuration; no provider request was dispatched.",
                ))
            })?;
        let response = self
            .reconciliation_http
            .execute(request)
            .await
            .map_err(|error| D1DatabaseRecoveryReadError {
                error: AdapterError::new(
                    ...
                    format!("Cloudflare D1 recovery read failed: {error}"),
                    "Retain reconciliation custody; provider state was not observed stably.",
                )
        ...
        if !identity_encoded {
            return Err(D1DatabaseRecoveryReadError {
                error: AdapterError::new(
                    "cloudflare.d1.database_recovery_unsupported_content_encoding",
                    "Cloudflare D1 recovery read used a non-identity content encoding",
                    "Retain reconciliation custody; provider state was not observed exactly.",
                )
            ...
            .is_some_and(|length| length > D1_MIGRATION_RESPONSE_MAX_BYTES as u64)
        {
            return Err(D1DatabaseRecoveryReadError {
                error: AdapterError::new(
                    "cloudflare.d1.database_recovery_response_too_large",
                    "Cloudflare D1 recovery response exceeded the exact-evidence byte limit",
                    "Retain reconciliation custody; provider state was not observed exactly.",
                )
                .with_status(Some(status_code))
                .payload(),
        ...
        while let Some(chunk) = stream.next().await {
            let chunk = chunk.map_err(|error| D1DatabaseRecoveryReadError {
                error: AdapterError::new(
                    "cloudflare.response_read_failed",
                    format!("failed reading Cloudflare D1 recovery response: {error}"),
                    "Retain reconciliation custody; provider state was not observed exactly.",
                )
                .with_status(Some(status_code))
                .with_retryable(false)
        ...
        let evidence_error = |error: AdapterError| D1DatabaseRecoveryReadError {
        ...
        let body = std::str::from_utf8(&bytes).map_err(|_| {
            evidence_error(AdapterError::new(
                "cloudflare.d1.database_recovery_malformed_utf8",
                "Cloudflare D1 recovery response was not valid UTF-8",
                "Retain reconciliation custody; provider evidence was malformed.",
            ))
        })?;
        let state = decode_strict_d1_database_recovery_envelope(body, status, database_id)
            .map_err(evidence_error)?;
        Ok(D1DatabaseRecoveryRead {
            state,
    ...
    pub(crate) async fn rename_d1_database_with_lifecycle(
        ...
        self.rename_d1_database_once_with_lifecycle(account_id, database_id, name, None)
            .await
    }

    pub(crate) async fn rename_d1_database_once_with_lifecycle(
        &self,
        account_id: &str,
        database_id: &str,
        name: &str,
        ...
        let mutation = self
            .execute_d1_database_mutation::<D1Database>(
                ...
                Some(json!({ "name": name })),
                D1DatabaseMutationEnvelopeContract::Rename,
                provider_request_id,
            )
            .await?;
        ...
        Ok(D1DatabaseMutation {
            ...
            response_body_sha256: mutation.response_body_sha256,
            response_body_size_bytes: mutation.response_body_size_bytes,
    ...
    pub(crate) async fn delete_d1_database_with_lifecycle(
        ...
        self.delete_d1_database_once_with_lifecycle(account_id, database_id, None)
            .await
    }

    pub(crate) async fn delete_d1_database_once_with_lifecycle(
        &self,
        account_id: &str,
        database_id: &str,
        provider_request_id: Option<&str>,
        ...
        let mutation = self
            .execute_d1_database_mutation::<Value>(
                ...
                None,
                D1DatabaseMutationEnvelopeContract::Delete,
                provider_request_id,
            )
            .await?;
        let status = mutation.lifecycle.http_status.unwrap_or(200);
    ...
    async fn execute_d1_database_mutation<T>(
        ...
        body: Option<Value>,
        envelope_contract: D1DatabaseMutationEnvelopeContract,
        provider_request_id: Option<&str>,
        ...
        T: DeserializeOwned,
    {
        ...
        let database_id = require_non_empty("database_id", database_id).map_err(pre_dispatch)?;
        let token = self.bearer_token().map_err(pre_dispatch)?;
        if provider_request_id.is_some_and(|value| !valid_d1_opaque_identity(value)) {
        ...
        if let Some(provider_request_id) = provider_request_id {
            request = request.header("x-client-request-id", provider_request_id);
        }
        ...
        while let Some(chunk) = stream.next().await {
            let chunk = chunk.map_err(|error| D1DatabaseMutationError {
                ...
                .with_status(Some(status_code))
                .with_retryable(false),
                response_body_sha256: (!bytes.is_empty()).then(|| sha256_bytes_hex(&bytes)),
                response_body_size_bytes: (!bytes.is_empty()).then_some(bytes.len()),
        ...
        let response_body_sha256 = sha256_bytes_hex(&bytes);
        let response_body_size_bytes = bytes.len();
        ...
        let result = decode_strict_d1_database_mutation_envelope::<T>(body, envelope_contract)
            .map_err(|error| D1DatabaseMutationError {
    ...
    /// Submit exactly one DML request through a bounded, no-redirect,
    /// non-idempotent boundary while retaining exact response-body evidence.
    pub(crate) async fn execute_d1_dml_write(
        ...
        let request = self
            ...
            .header(reqwest::header::USER_AGENT, self.cfg.user_agent.clone())
            .header(reqwest::header::ACCEPT_ENCODING, "identity")
            .header("x-client-request-id", provider_request_id)
        ...
        let response = self.migration_write_http.execute(request).await.map_err(|error| {
            let code = if error.is_timeout() { "cloudflare.timeout" } else { "cloudflare.transport_error" };
            D1DmlWriteError {
                error: AdapterError::new(
                    code,
        ...
        if !identity_encoded {
            return Err(D1DmlWriteError {
                error: AdapterError::new(
                    ...
                    "Treat the DML outcome as ambiguous; retain custody and do not replay the attempt.",
                ).with_status(Some(status_code)).payload(),
                provider_error: None,
                response_body_sha256: None,
                response_body_size_bytes: None,
            ...
            .is_some_and(|length| length > D1_MIGRATION_RESPONSE_MAX_BYTES as u64)
        {
            return Err(D1DmlWriteError {
                ...
                provider_error: None,
                response_body_sha256: None,
                response_body_size_bytes: response.content_length().map(|length| usize::try_from(length).unwrap_or(usize::MAX)),
                lifecycle: D1DmlWriteLifecycle::response_received(status_code),
            });
        }
        let initial_capacity = response
        ...
        while let Some(chunk) = stream.next().await {
            let chunk = chunk.map_err(|error| D1DmlWriteError {
                error: AdapterError::new(
                    ...
                    "Treat the DML outcome as ambiguous; retain custody and do not replay the attempt.",
                ).with_status(Some(status_code)).with_retryable(false).payload(),
                provider_error: None,
                response_body_sha256: None,
                response_body_size_bytes: Some(bytes.len()),
...
fn decode_strict_d1_dml_envelope(body: &str) -> Result<CloudflareEnvelope<Value>, AdapterError> {
    ...
    match envelope.errors {
        ...
        _ => {
            return Err(AdapterError::new(
                "cloudflare.d1.execute_write_malformed_envelope",
                "Cloudflare DML envelope omitted an exact empty errors array",
                "Treat the DML outcome as ambiguous; retain custody and do not replay the attempt.",
            ));
        }
    }
...
fn deserialize_present_non_null_option<'de, D, T>(deserializer: D) -> Result<Option<T>, D::Error>
where
    D: serde::Deserializer<'de>,
    T: Deserialize<'de>,
{
...
fn decode_strict_d1_database_mutation_envelope<T>(
    body: &str,
    contract: D1DatabaseMutationEnvelopeContract,
) -> Result<T, AdapterError>
where
    T: DeserializeOwned,
{
    ...
    let malformed = || {
        AdapterError::new(
            "cloudflare.d1.database_mutation_malformed_envelope",
            "Cloudflare D1 database mutation response did not match the exact operation envelope",
            "Treat the database mutation outcome as ambiguous; do not retry this attempt.",
        )
    };
    let (success, result, errors, messages) = match contract {
        D1DatabaseMutationEnvelopeContract::Rename => {
            let envelope: StrictD1DatabaseRenameEnvelope =
        ...
        D1DatabaseMutationEnvelopeContract::Delete => {
            let envelope: StrictD1DatabaseDeleteEnvelope =
                serde_json::from_value(value).map_err(|_| malformed())?;
            (
                envelope.success,
                envelope.result.unwrap_or_else(|| json!({})),
                envelope.errors,
                envelope.messages,
            )
        }
...
fn decode_strict_d1_database_recovery_envelope(
    body: &str,
    status: StatusCode,
    expected_database_id: &str,
) -> Result<D1DatabaseRecoveryState, AdapterError> {
    let malformed = || {
        AdapterError::new(
            "cloudflare.d1.database_recovery_malformed_envelope",
            "Cloudflare D1 recovery response did not match the exact readback envelope",
            "Retain reconciliation custody; provider evidence was malformed or insufficient.",
        )
        .with_status(Some(status.as_u16()))
    };
    let value = decode_json_rejecting_duplicate_object_keys(body).map_err(|_| malformed())?;
    let envelope: StrictD1DatabaseRecoveryEnvelope =
        serde_json::from_value(value).map_err(|_| malformed())?;
    let messages_empty = matches!(&envelope.messages, Value::Array(values) if values.is_empty());
    if !messages_empty {
        return Err(malformed());
    }
    if status.is_success() {
        ...
        let result: StrictD1DatabaseRecoveryResult =
        ...
        return Ok(D1DatabaseRecoveryState::Present {
            uuid: result.uuid,
            name: result.name,
    ...
    if status == StatusCode::NOT_FOUND && !envelope.success && envelope.result.is_none() {
        ...
        {
            return Ok(D1DatabaseRecoveryState::Absent);
        }
        return Err(malformed());
    }
    Err(AdapterError::new(
        "cloudflare.d1.database_recovery_unavailable",
        "Cloudflare D1 recovery read did not prove present or absent state",
        "Retain reconciliation custody; do not infer a terminal outcome from this response.",
    )
    .with_status(Some(status.as_u16())))
}
...
mod tests {
    ...
    use super::{
        ...
        D1_MIGRATION_RESPONSE_MAX_BYTES, D1DatabaseRecoveryState, D1DmlWriteLifecycle,
        D1MigrationManifestWriteLifecycle, D1MigrationProviderError,
        D1MigrationProviderErrorLocation, D1MigrationReconciliationReadLifecycle,
        DuplicateSafeJsonError, classify_d1_migration_provider_error,
        decode_json_rejecting_duplicate_object_keys, decode_strict_d1_database_recovery_envelope,
    ...
    impl TestD1DatabaseMutation {
        ...
        fn exact_result_json(self) -> &'static str {
            match self {
                Self::Rename => {
                    r#"{"name":"renamed-db","uuid":"123e4567-e89b-42d3-a456-426614174000"}"#
                }
                Self::Delete => r#"{"deleted":true}"#,
            }
        }
    }

    #[test]
    fn strict_d1_database_recovery_decoder_closes_present_absent_and_malformed_products() {
        let database_id = "123e4567-e89b-42d3-a456-426614174000";
        let present = format!(
            r#"{{"success":true,"result":{{"uuid":"{database_id}","name":"renamed-db","version":"production"}},"errors":[],"messages":[]}}"#
        );
        assert!(matches!(
            decode_strict_d1_database_recovery_envelope(
                &present,
                StatusCode::OK,
                database_id
            ),
            Ok(D1DatabaseRecoveryState::Present { name, .. }) if name == "renamed-db"
        ));
        let absent = r#"{"success":false,"result":null,"errors":[{"code":7404,"message":"database not found"}],"messages":[]}"#;
        assert_eq!(
            decode_strict_d1_database_recovery_envelope(absent, StatusCode::NOT_FOUND, database_id)
                .expect("exact missing target evidence"),
            D1DatabaseRecoveryState::Absent
        );
        for malformed in [
            r#"{"success":true,"success":false,"result":null,"errors":[],"messages":[]}"#,
            r#"{"success":false,"result":null,"errors":[{"code":7003,"message":"wrong context"}],"messages":[]}"#,
            r#"{"success":true,"result":{"uuid":"different","name":"renamed-db"},"errors":[],"messages":[]}"#,
        ] {
            assert!(
                decode_strict_d1_database_recovery_envelope(
                    malformed,
                    if malformed.contains("7003") {
                        StatusCode::NOT_FOUND
                    } else {
                        StatusCode::OK
                    },
                ...
                .is_err(),
                "malformed or wrong-context evidence must fail closed: {malformed}"
            );
        }
    }

    fn strict_d1_database_mutation_failure_bodies(
        operation: TestD1DatabaseMutation,
        ...
        let exact_result = operation.exact_result_json();
        let exact_errors = "[]";
        let exact_messages = "[]";
        ...
        let mut cases = vec![
            ("malformed-json", b"{".to_vec(), malformed_code),
            ...
            (
                ...
                envelope(
                    ...
                    exact_result,
                    exact_errors,
                    r#"[{"message":"must-not-return"}]"#,
                ),
                malformed_code,
            ),
            (
                "message-array-nonobject",
                envelope("true", exact_result, exact_errors, r#"[7]"#),
                malformed_code,
            ),
            (
                "message-null-code",
                envelope(
                    ...
                    exact_result,
                    exact_errors,
                    r#"[{"code":null,"message":"must-not-return"}]"#,
                ),
                malformed_code,
            ),
            (
                ...
                envelope(
                    ...
                    exact_result,
                    exact_errors,
                    r#"[{"code":"1000","message":"must-not-return"}]"#,
                ),
                malformed_code,
            ),
            (
            ...
            (
                ...
                envelope(
                    ...
                    exact_result,
                    exact_errors,
                    r#"[{"code":999,"message":"must-not-return"}]"#,
                ),
                malformed_code,
            ),
            (
                "message-missing-message",
                envelope("true", exact_result, exact_errors, r#"[{"code":1000}]"#),
                malformed_code,
            ),
            (
            ...
            (
                ...
                envelope(
                    ...
                    exact_result,
                    exact_errors,
                    r#"[{"code":1000,"message":7}]"#,
                ),
                malformed_code,
            ),
            (
                "message-null-documentation-url",
                envelope(
                    "true",
                    exact_result,
                    exact_errors,
                    r#"[{"code":1000,"message":"must-not-return","documentation_url":null}]"#,
                ),
                malformed_code,
            ),
            (
                "message-wrong-documentation-url-type",
                envelope(
                    "true",
                    exact_result,
                    exact_errors,
                    r#"[{"code":1000,"message":"must-not-return","documentation_url":7}]"#,
                ),
                malformed_code,
            ),
            (
                ...
                envelope(
                    ...
                    exact_result,
                    exact_errors,
                    r#"[{"code":1000,"message":"must-not-return","source":null}]"#,
                ),
                malformed_code,
            ),
            (
                "message-wrong-source-type",
                envelope(
                    "true",
                    exact_result,
                    exact_errors,
                    r#"[{"code":1000,"message":"must-not-return","source":"/pointer"}]"#,
                ),
                malformed_code,
            ),
            (
                "message-source-null-pointer",
                envelope(
                    "true",
                    exact_result,
                    exact_errors,
                    r#"[{"code":1000,"message":"must-not-return","source":{"pointer":null}}]"#,
                ),
                malformed_code,
            ),
            (
                "message-source-wrong-pointer-type",
                envelope(
                    "true",
                    exact_result,
                    exact_errors,
                    r#"[{"code":1000,"message":"must-not-return","source":{"pointer":7}}]"#,
                ),
                malformed_code,
            ),
            (
                "message-source-unknown-field",
                envelope(
                    "true",
                    exact_result,
                    exact_errors,
                    r#"[{"code":1000,"message":"must-not-return","source":{"detail":"hidden"}}]"#,
                ),
                malformed_code,
            ),
            (
            ...
            (
                "duplicate-nested-message-field",
                envelope(
                    "true",
                    exact_result,
                    exact_errors,
                    r#"[{"code":1000,"code":1001,"message":"must-not-return"}]"#,
                ),
                duplicate_code,
            ),
            (
                "duplicate-nested-message-source-field",
                envelope(
                    "true",
                    exact_result,
                    exact_errors,
                    r#"[{"code":1000,"message":"must-not-return","source":{"pointer":"/a","pointer":"/b"}}]"#,
                ),
                duplicate_code,
            ),
            (
        ...
        if matches!(operation, TestD1DatabaseMutation::Rename) {
            cases.extend([
                (
                    "missing-result",
                    format!(
    ...
    async fn d1_database_mutations_keep_response_contract_failures_uncertain_without_retry() {
        for operation in [
            ...
            for (label, body, expected_code) in
                strict_d1_database_mutation_failure_bodies(operation)
            {
    ...
    #[tokio::test]
    async fn d1_database_mutations_accept_exact_response_info_messages() {
        for operation in [
            TestD1DatabaseMutation::Rename,
            TestD1DatabaseMutation::Delete,
        ] {
            ...
            let (base, calls) = spawn_single_raw_d1_database_mutation_response(Some(
                raw_http_response(200, body.as_bytes()),
            ))
            .await;
            let client = CloudflareClient::new(test_config(base)).expect("mutation client");
            let (_, lifecycle) = operation
                .call(&client)
                .await
                .expect("valid ResponseInfo messages preserve success");
            assert_eq!(
                lifecycle,
    ...
    #[tokio::test]
    async fn d1_database_delete_normalizes_absent_or_null_result_and_preserves_present_json() {
        let cases = [
            (
                "absent",
            ...
            (
                "null",
                br#"{"success":true,"result":null,"errors":[],"messages":[]}"#.as_slice(),
                json!({}),
            ),
            (
                "scalar",
                br#"{"success":true,"result":7,"errors":[],"messages":[]}"#.as_slice(),
                json!(7),
            ),
            (
                "array",
                br#"{"success":true,"result":["deleted"],"errors":[],"messages":[]}"#.as_slice(),
                json!(["deleted"]),
            ),
            (
                "object",
                br#"{"success":true,"result":{"deleted":true},"errors":[],"messages":[]}"#
                    .as_slice(),
                json!({"deleted": true}),
            ),
        ];
        for (label, body, expected_result) in cases {
            let (base, calls) =
                spawn_single_raw_d1_database_mutation_response(Some(raw_http_response(200, body)))
                    .await;
            let client = CloudflareClient::new(test_config(base)).expect("mutation client");
            let (result, lifecycle) = TestD1DatabaseMutation::Delete
                .call(&client)
                .await
                .unwrap_or_else(|_| panic!("delete {label} result succeeds"));
            assert_eq!(result, expected_result, "{label}");
            assert_eq!(lifecycle, D1DatabaseMutationLifecycle::succeeded(200));
            assert_eq!(lifecycle.provider_calls(), 1, "{label}");
            assert_eq!(lifecycle.provider_mutations(), Some(1), "{label}");
