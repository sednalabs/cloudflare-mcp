...
pub fn list_resources(
    ...
    vec![
        resource_for_text(
            ...
            "about",
            "Cloudflare MCP",
            "Purpose, scope, and safety boundaries.",
            MIME_MARKDOWN,
            Some(about_text.len()),
        ),
        resource_for_text(
...
fn build_about_text() -> String {
    [
        ...
        "",
        "Purpose:",
        "- Provide self-hosted, guarded Cloudflare operator workflows.",
        "- Exercise reusable Rust MCP Toolkit behavior against realistic Cloudflare workloads before suitable primitives are upstreamed.",
        "",
        "Project scope:",
        "- Keep local capabilities when they add concrete operator value or meaningful Toolkit incubation/conformance value.",
        "- Prefer Cloudflare's official MCPs for broad/current API access and specialist product surfaces when neither local reason applies.",
        "",
        "Non-goal:",
        "- Do not duplicate Cloudflare's managed MCP catalog solely for endpoint coverage.",
        "",
        "Security posture:",
...
fn build_help_text() -> String {
    [
        ...
        "- OpenAI Responses API clients that support MCP deferred loading and tool search can combine MCP `defer_loading=true` with `{ \"type\": \"tool_search\" }`; consult current OpenAI product documentation for model/client support.",
