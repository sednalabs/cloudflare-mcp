...
fn d1_target_wide_rename_dispatches_once_and_exact_replay_never_redispatches() {
    ...
    assert_eq!(
        first["custody"]["causal_authority"],
        json!({
            "dispatch_status": "authenticated_acknowledgement",
            "state_observation": "not_observed",
            "causality": "unproven",
    ...
    assert_eq!(
        replay["custody"]["causal_authority"],
        json!({
            "dispatch_status": "authenticated_acknowledgement",
            "state_observation": "not_observed",
            "causality": "unproven",
...
fn d1_target_wide_malformed_response_is_retained_and_never_retried() {
    ...
    assert_eq!(
        first["custody"]["causal_authority"],
        json!({
            "dispatch_status": "uncertain_after_dispatch",
            "state_observation": "not_observed",
            "causality": "unproven",
            "current_state_can_authorize": false,
            "terminalization_authorized": false
    ...
    assert_eq!(
        structured_content(&replay)["custody"]["causal_authority"],
        json!({
            "dispatch_status": "uncertain_after_dispatch",
            "state_observation": "not_observed",
            "causality": "unproven",
...
fn d1_execute_write_compound_classifier_denials_are_pre_provider() {
    ...
    for (request_id, sql) in [
        (
            16,
            "INSERT INTO example(id) VALUES (?) ON CONFLICT(id) DO NOTHING",
        ),
        (
            17,
            "INSERT INTO example(id) VALUES (?) ON\nCONFLICT(id) DO\tUPDATE",
        ),
        (
            18,
            "INSERT INTO example(id) VALUES (?) ON/* unsupported */CONFLICT(id) DO UPDATE SET id=?",
        ),
        (
            19,
        ...
        let response = mcp.call_tool(
            ...
            json!({
                "database_id": "123e4567-e89b-42d3-a456-426614174000",
                "sql": sql,
                "operation_id": format!("operation-classifier-{request_id}"),
                "execution_attempt_id": format!("attempt-classifier-{request_id}"),
                "provider_request_id": format!("provider-classifier-{request_id}"),
                "dry_run": true
            }),
        );
        let content = structured_content(&response);
        ...
        assert!(
            matches!(provider.accept(), Err(error) if error.kind() == std::io::ErrorKind::WouldBlock),
            "compound classifier denial must occur before any provider connection: {sql}"
        );
    }
