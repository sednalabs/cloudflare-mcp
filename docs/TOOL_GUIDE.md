...
| Create one disabled Worker version | `workers_upload_version` | apply | One reviewed inline artifact, or a canonical relative file path beneath the private `CLOUDFLARE_MCP_WORKER_UPLOAD_ROOT`; metadata containing exactly main module, compatibility date, duplicate-free compatibility flags and a complete bindings array; exact base version ID and ETag; pinned pre-upload version/deployment snapshot hashes; mandatory `bindings_inherit="strict"`; dry-run confirmation; canonical private shared `CLOUDFLARE_MCP_WORKER_VERSION_ATTEMPT_ROOT` for live apply | Descriptor-relative, no-follow regular-file read under an owner-only root; confirmation-bound request/body/normalized-metadata digests; explicit digest-only request evidence (`exact_request_bytes_retained=false`, `canonical_request_manifest_retained=false`); D1 alias and AI Search default normalization; pre-state equality plus a second pinned-state capture while the shared attempt guard is held; append-only prepared/dispatched/terminal attempt authority with dispatched fsynced before POST; closed three-entry restored namespace; nonblocking no-follow descriptor read with regular/private/bounded fstat before read, capped bytes and same-descriptor revalidation; one non-retrying POST; truthful request lifecycle; exact candidate ID/ETag and request/response artifact SHA-256 digests; fully typed script/runtime/version-metadata readback projections; complete secret-safe binding projection matched to explicit metadata plus exact-base inheritance; sorted strategy-aware unchanged deployment proof; explicit `candidate_created_digest_only` with `source_provider_unverified` rather than request-byte custody or provider source-byte proof | Unknown arguments or metadata, export reconciliation, migrations, assets/cache/settings/runtime controls, missing binding plan, absent/unsafe attempt root, retained prepared/dispatched/terminal attempt, concurrent same or distinct confirmed invocation, stale guarded pre-state, FIFO/socket/device/special-file receipt, dangling symlink, extra or oversized namespace, conflicting or malformed custody, restart/replay, response loss before or after visibility, absolute/parent/symlink/nonregular/swap path substitution, unknown binding types/fields, provider detail that retains unresolved `inherit`, conflicting D1 aliases, semantic runtime/binding drift, missing confirmation, wrong base, `latest`/implicit inherit, pre-state drift, malformed detail, missing ID/ETag, cross-target detail, secret-shaped outward data, candidate deployed | Implemented and focused-tested |
