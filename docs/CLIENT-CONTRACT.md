...
| `d1_rename_database` | `database_id`, `name`, pairwise-distinct bounded `operation_id`, `execution_attempt_id`, `provider_request_id`; `account_id` unless default account configured | `dry_run`, `confirmation_token`, `reason` | Curated guarded D1 rename with a closed input schema. Dry-run returns the complete static eight-step plan, explicit versioned consent binding, and `required_confirmation_token`; the token binds normalized target, rename, reason, operation/version, plan digest, and full plan. Live execution independently rederives that canonical product before guard acquisition and derives the exact PATCH path/body solely from it; there is no separate caller-selected provider operation. Mismatch or impossible extraction is a zero-call denial. Live execution then requires exact consent, a clean complete custody audit before fresh `Prepared` creation, one exact Prepared owner plus Bound claimants, immediate owner-aware revalidation, and exactly one `Prepared -> DispatchReserved` CAS before at most one no-redirect provider request carrying the preallocated provider-request identity. A fresh layout ensure reports `created`/one local mutation, an incumbent reports `already_present`/zero, and failure reports failed/unknown local state with zero provider calls. Strict authenticated acknowledgement is durably recorded as `Acknowledged`; transport loss, malformed/contradictory response, adapter failure, or uncertain provider evidence is recorded as `ReconciliationRequired`. Both outcomes are nonterminal, forbid automatic retry, and exact replay never redispatches. Any pre-reservation guard, audit, identity, version, capacity, or CAS failure returns zero provider calls. Post-provider derivation, CAS, or readback uncertainty retains aggregate response/lifecycle evidence and requires separately governed recovery/finalization. RMCP text and structured JSON are whole-payload equivalent after final audit/evidence insertion. |
