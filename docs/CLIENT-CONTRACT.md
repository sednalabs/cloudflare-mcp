...
| `workers_upload_version` | `script_name`, exact `base_version_id` and `base_version_etag`, pinned pre-upload version/deployment snapshots, `bindings_inherit="strict"`, closed complete metadata, and exactly one reviewed module or multipart source; `account_id` unless default account configured | `main_module`, `content_type`, `per_page` (1–100), exactly one of `dry_run=true`, `prepare=true`, or `approval_handle`, plus `reason` | Creates one disabled Worker version and never creates a deployment. Preview validates the complete candidate in memory with zero local/provider mutation and returns no actionable token. Preparation is an explicit local mutation: it requires a canonical owner-only `CLOUDFLARE_MCP_WORKER_VERSION_APPROVAL_ROOT` whose entire canonical ancestor chain is root/current-user owned and not group/world writable, creates one bounded private namespace under a kernel-random opaque handle, persists the exact body and canonical metadata with create-only descriptor/fsync discipline, and returns only the handle plus non-sensitive lifecycle facts. No deterministic candidate digest, candidate-derived size, private path, receipt bytes, or secret value is public. The tool is excluded from generic argument-digest/preview elicitation, including explicit configuration, because that pre-handler path would expose an oracle over secret-bearing arguments; its private random-handle ceremony is its sole approval mechanism. Apply reloads and re-reads the private candidate under held guards immediately before consume, requires byte-for-byte equality with the complete caller candidate, and durably consumes approval before any provider access. A first mismatch durably rejects the handle before returning and it can never be guessed or retried. Approval and dispatch-attempt objects retain their configured root descriptors and revalidate root plus child reachability before and after every transition. At the hard namespace cap, preparation fails with bounded `custody_capacity` evidence and `safe_to_rotate=false`; operator rotation requires a bounded terminal-only audit followed by a create-only generation fence installed, fsynced, and read back under the exclusive root guard before archival or cutover. In-flight loads retain a shared root guard, and every prepare/load/consume rejects a retired root. Only then may terminal evidence be archived and all processes atomically switched to a fresh trusted root. Expired, consumed, retired, rejected, concurrent, absent, conflicting, malformed, noncanonical, oversized, symlink, FIFO, socket, device, unexpected-entry, root-drift, restart, and replay evidence fails closed; physical presence is never treated as absence. Once separate downstream dispatch-attempt custody exists, approval becomes retired before the one non-retrying provider POST. The existing `CLOUDFLARE_MCP_WORKER_VERSION_ATTEMPT_ROOT` remains a distinct prepared/dispatched/terminal authority for provider ambiguity and never inherits approval authority. Apply proves one exact disabled candidate with matching runtime/bindings and unchanged deployments. Cloudflare exposes no atomic compare-and-create precondition between the final provider read and POST; local one-use custody prevents duplicate replay without claiming that interval is atomic. Public success exposes only the provider candidate ID and non-sensitive boolean/lifecycle facts; request, response, upload-contract, metadata, projection and source-derived digests/sizes remain private. Response loss, process death, provider rejection, malformed readback, drift, or verification conflict never makes consumed approval reusable. |
| `workers_reconcile_version_upload` | `script_name`, exact base ID/ETag, pinned pre-upload version IDs and their SHA-256, pinned pre-upload deployment projection and SHA-256; `account_id` unless default account configured | `per_page` | Read-only response-loss evidence collection. It enumerates the current stable version set against the exact pinned predecessor set. It accepts and returns no candidate-derived upload-contract digest: provider inventory cannot bind one to the lost request without private custody. Even when there is exactly one new disabled version, a still-present exact base, and unchanged deployments, provider inventory alone cannot prove that the lost request created that candidate or that its complete bytes and binding plan match the reviewed private candidate; the result remains `reconciliation_required` with `attribution_state=unattributed`. No or multiple candidates, a missing predecessor, supplied snapshot-hash mismatch, state drift, or a candidate in a deployment also fails closed. It never uploads or creates a deployment and never authorizes replay of the original POST. |
...
- Broad cache actions add local confirmation: `cache_purge` with `payload.everything=true` and `cache_rules` with `action=replace_all` require echoing the token returned by dry-run. `workers_upload_script` also uses that deterministic token contract. `workers_upload_version` instead requires an explicit create-only private preparation phase and one kernel-random opaque handle because its complete metadata may contain low-entropy secrets; apply reloads and consumes the exact private candidate rather than exposing a deterministic confirmation oracle. It additionally requires exact stable pre-state pins, strict exact-base binding inheritance, and proves that the new candidate is not deployed. `waf_ruleset_apply_change` requires the token returned by `waf_ruleset_plan_change` because WAF action, ordering, list, and cap mistakes can affect production traffic.
