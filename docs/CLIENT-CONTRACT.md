...
provider envelope. A future adapter must normalize two physical observations
into the exact versioned projection payload and must bind each observation to
the same canonical target and rederived plan using four distinct preallocated
dispatch/read identities.
...
strictly ordered exactly as the fixed query specifies. The two complete row
...
triggers, foreign keys, implicit writes, DML, provider dispatch, custody, or any
