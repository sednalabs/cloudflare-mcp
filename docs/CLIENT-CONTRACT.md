...
| `d1_provision_dml_custody` | `database_id`, one opaque `custody_generation`, independently supplied `authority_sha256`; `account_id` unless default account configured | `dry_run`, `approved_plan_sha256` (required live) | Provider-free one-time provisioning for immutable target-local DML custody genesis and its generation-bound layout. Supplied authority must exactly match `CLOUDFLARE_MCP_D1_CUSTODY_GENERATION` and `CLOUDFLARE_MCP_D1_CUSTODY_AUTHORITY_SHA256`. First application returns `applied`; exact replay returns `proven`; changed, partial, malformed, replaced, or orphaned evidence fails closed with zero provider calls and no repair. Ordinary D1 mutation tools never create either product. |
