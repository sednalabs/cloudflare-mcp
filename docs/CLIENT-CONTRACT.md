...
| `d1_rename_database` | `database_id`, `name`, pairwise-distinct bounded `operation_id`, `execution_attempt_id`, `provider_request_id`; `account_id` unless default account configured | `dry_run`, `confirmation_token`, `reason` | Curated guarded D1 rename with a closed input schema. Dry-run returns the complete static eight-step plan, explicit versioned consent binding, and `required_confirmation_token`; the token binds normalized target, rename, reason, operation/version, plan digest, and full plan. Live execution independently rederives that canonical product before guard acquisition and derives the exact PATCH path/body solely from it; there is no separate caller-selected provider operation. Mismatch or impossible extraction is a zero-call denial. Live execution then requires exact consent, a clean complete custody audit before fresh `Prepared` creation, one exact Prepared owner plus Bound claimants, immediate owner-aware revalidation, and exactly one `Prepared -> DispatchReserved` CAS before at most one no-redirect provider request carrying the preallocated provider-request identity. Strict authenticated acknowledgement is durably recorded as `Acknowledged`; uncertain evidence is recorded as `ReconciliationRequired`; neither authorizes retry. Exact replay of reserved/reconciliation custody never redispatches: it performs two bounded authenticated no-redirect GETs and terminalizes only when both strict states match. A stable present target with the requested name proves applied; a stable present target with another name proves not applied; absence is insufficient. The terminal CAS binds current target/plan/consent versions, all three identities, incumbent aggregate provider evidence, and both response digests/statuses. Unstable, redirected, malformed, missing, contradictory, or insufficient evidence remains reconciliation-required. Terminal replay performs zero provider calls and terminal identities remain permanently bound. RMCP text and structured JSON are whole-payload equivalent. |
| `d1_delete_database` | `database_id`, pairwise-distinct bounded `operation_id`, `execution_attempt_id`, `provider_request_id`; `account_id` unless default account configured | `dry_run`, `confirmation_token`, `reason` | Curated high-risk D1 delete with the same plan-derived closed provider-request authority, eight-step consent, exact Prepared-owner authorization, exact layout-outcome evidence, single dispatch-reservation CAS, one-request maximum, strict response-envelope, aggregate-only evidence, and permanent no-automatic-retry contract as rename. Exact replay never redispatches; reserved/reconciliation custody performs two strict authenticated no-redirect GETs. Matching 404 absence proves applied, while matching exact present metadata proves not applied. Target, operation/version, delete intent, reason, plan, identity, incumbent evidence, readback, or terminal-CAS drift fails closed and preserves reconciliation custody. |
...
`ReconciliationRequired`. Neither outcome authorizes retry. Exact replay from
reserved or reconciliation custody performs two authenticated no-redirect
reads and may advance once to immutable `TerminalApplied` or
`TerminalNotApplied`; exact terminal replay performs no provider read or write.
