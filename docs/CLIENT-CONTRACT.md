...
| `d1_execute_write` | `database_id`, `sql`; `account_id` unless default account configured | `params`, `dry_run`, `max_rows` | Executes one audited D1 row-write statement after dry-run planning. Allows only single-statement `INSERT`, `UPDATE`, `DELETE`, or `REPLACE`; schema-changing migration SQL belongs in `d1_apply_migration_manifest`. The current public route does not yet consume the staged version-2 catalog-backed reserved-relation authority proof, so it must not be used on migration-managed targets until that separate integration boundary lands. The staged pre-provider classifier derives one fixed full table/view/trigger catalog query with `LIMIT 1001` and requires two stable primary-served, read-only snapshots. Each integration evidence envelope must name the plan-bound provider row cap `1001` and literal `results_truncated=false`; the existing generic 1000-row read helper cannot satisfy this authority contract. A returned 1001st row is the overflow sentinel. Table definitions require a parenthesized body and a fully consumed suffix limited to the supported `STRICT`/`WITHOUT ROWID` option grammar; CTAS and unknown tails fail closed. Every `REFERENCES` tail is fully consumed under the narrow parent/column/`ON DELETE`/`ON UPDATE` grammar; unsupported or malformed clauses do not silently become `NO ACTION`. The classifier validates every configured migration ledger against the exact supported trigger-free schema and follows parsed table/view trigger effects plus SQLite-implicit writes transitively. INSERT/REPLACE into an `AUTOINCREMENT` table is treated as reaching `sqlite_sequence`. Foreign-key `CASCADE`, `SET NULL`, and `SET DEFAULT` actions add child write edges for their exact parent event; `RESTRICT` and `NO ACTION` remain constraint-only and add no write edge. Direct, quoted, schema-qualified, trigger-mediated, or implicit access to configured ledger identities, SQLite `sqlite_*`, or Cloudflare `_cf_*` relations fails closed. Missing, truncated, non-TEXT, duplicate, malformed, oversized, non-primary, mutating, or unstable catalog evidence does not authorize provider dispatch. This internal proof owns no provider call, custody, recovery, or public route behavior. |
