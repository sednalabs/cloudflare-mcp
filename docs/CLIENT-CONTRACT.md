...
- Review `request_plan.body_encoding` as part of confirmation. The generic
  `worker-script-patch-settings` operation reports `multipart_form_data` and
  encodes the same normalized logical body as the `settings` JSON part during
  apply. Its generated multipart boundary is not confirmation authority and is
