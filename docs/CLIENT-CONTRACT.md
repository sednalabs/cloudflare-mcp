...
| `d1_rename_database` | `database_id`, `name`; `account_id` unless default account configured | `dry_run`, `confirmation_token`, `reason` | Curated D1 rename planner. Dry-run returns the complete static plan, explicit versioned consent binding, and `required_confirmation_token`. The token binds the normalized target, rename, reason, operation/version, plan digest, and full plan. Missing or stale live consent fails before guard/provider access; exact consent currently returns `durable_reservation_not_installed` with zero provider calls because live execution remains disabled pending durable reservation/recovery custody. |
| `d1_delete_database` | `database_id`; `account_id` unless default account configured | `dry_run`, `confirmation_token`, `reason` | Curated high-risk D1 delete planner with the same complete plan-bound consent contract. Target, operation/version, delete intent, reason, plan digest, and full plan changes invalidate the token. Missing or stale live consent fails before guard/provider access; exact consent currently returns `durable_reservation_not_installed` with zero provider calls because live execution remains disabled pending durable reservation/recovery custody. |
