...
| `d1_execute_write` | `database_id`, `sql`, `execution_session_sha256`; `account_id` unless default account configured | `params`, `dry_run`, `approved_plan_sha256` (required live), `max_rows` | Executes exactly one audited D1 row-write statement under durable attempt custody. The dry-run digest binds the exact SQL bytes (including surrounding whitespace), canonical parameters, target, statement kind, row cap, and opaque execution session. Live apply requires that exact digest, performs at most one no-redirect provider request, and reports physical call/mutation counts plus response status/body digest/body size lifecycle evidence. A primary-served zero-change `UPDATE` or `DELETE` is terminal success; zero-change `INSERT` or `REPLACE`, malformed/oversized/truncated/invalid responses, authenticated non-success, and response loss retain custody for reconciliation and never permit automatic retry. Reusing the same session and exact completed plan returns a zero-call terminal replay; changing the plan conflicts. Allows only single-statement `INSERT`, `UPDATE`, `DELETE`, or `REPLACE`; schema-changing migration SQL belongs in `d1_apply_migration_manifest`. |
