...
| `d1_provision_dml_custody` | `database_id`, one opaque `custody_generation`, independently supplied `authority_sha256`; `account_id` unless default account configured | `dry_run`; retained `approved_plan_sha256` is compatibility-only and never grants live authority | Read-only planner for an external D1 custody entitlement. `dry_run=true` returns exact target/genesis/layout requirements; every live invocation returns `d1.dml_custody_offline_provisioning_required` with zero local/provider effects. Actual create-once authority exists only in the explicit offline CLI process with an operator-owned seal root outside the lease root. Its in-progress consumption and final consumed receipt bind the virgin root identity, canonical target, generation/authority, operation, exact local readback, and replay state. Resume may create only from the still-virgin root or prove an already-complete exact product; partial local products are never repaired. Ordinary D1 mutation tools never create or repair these products. |
| `d1_rename_database` | `database_id`, `name`, pairwise-distinct bounded `operation_id`, `execution_attempt_id`, `provider_request_id`; `account_id` unless default account configured | `dry_run`, `confirmation_token`, `reason` | Curated guarded D1 rename with a closed input schema. Dry-run returns the complete static eight-step plan, explicit versioned consent binding, and `required_confirmation_token`; the token binds normalized target, rename, reason, operation/version, plan digest, and full plan. Live execution independently rederives that canonical product before guard acquisition and derives the exact PATCH path/body solely from it. Live execution opens exact pre-provisioned genesis and layout, requires exact consent and a clean complete custody audit before fresh generation-bound `Prepared` creation, proves one exact Prepared owner plus Bound claimants, immediately revalidates owner authority, and performs exactly one `Prepared -> DispatchReserved` CAS before at most one no-redirect provider request. Missing, malformed, replaced, partial, orphaned, or generation-mismatched custody reports zero provider calls and is never recreated. Strict authenticated acknowledgement is durably recorded as `Acknowledged`; transport loss, malformed/contradictory response, adapter failure, or uncertain provider evidence is recorded as `ReconciliationRequired`. Both outcomes are nonterminal, forbid automatic retry, and exact replay never redispatches. Any pre-reservation guard, audit, identity, version, capacity, or CAS failure returns zero provider calls. Post-provider derivation, CAS, or readback uncertainty retains aggregate response/lifecycle evidence and requires separately governed recovery/finalization. RMCP text and structured JSON are whole-payload equivalent after final audit/evidence insertion. |
| `d1_delete_database` | `database_id`, pairwise-distinct bounded `operation_id`, `execution_attempt_id`, `provider_request_id`; `account_id` unless default account configured | `dry_run`, `confirmation_token`, `reason` | Curated high-risk D1 delete with the same plan-derived closed provider-request authority, eight-step consent, exact Prepared-owner authorization, exact layout-outcome evidence, single dispatch-reservation CAS, one-request maximum, strict response-envelope, nonterminal `Acknowledged`/`ReconciliationRequired` custody, aggregate-only evidence, and permanent no-automatic-retry contract as rename. Target, operation/version, delete intent, reason, plan digest, full plan, or identity drift fails closed; exact replay cannot redispatch. Stable provider recovery, effect readback, and terminal finalization remain a separate reviewed lifecycle. |
...
| `d1_execute_write` | `database_id`, `sql`, pairwise-distinct bounded `operation_id`, `execution_attempt_id`, `provider_request_id`; `account_id` unless default account configured | `params`, `dry_run`, `max_rows`, `approved_composition_sha256` (required live) | Plans or reserves and dispatches one exact audited D1 row-write. The three opaque identities must each be 16–128 ASCII bytes containing only letters, digits, `.`, `_`, `:`, or `-`, and must be pairwise distinct; the public schema rejects unknown arguments. Audit custody starts before caller-controlled account, target, or identity validation. Every handler-owned pre-provider denial returns the same typed blocked lifecycle envelope with `provider_calls=0`, `provider_mutations=0`, aggregate-only target evidence, and whole-payload-equivalent RMCP text and structured JSON. The closed bounded-token classifier accepts one unquoted canonical lowercase relation in supported `INSERT`, `UPDATE`, `DELETE`, `REPLACE`, and explicitly classified replacement/upsert forms. A relation starts with lowercase ASCII and may continue with lowercase ASCII, digits, `_`, or `$`; `$` remains part of the exact relation identity and cannot collapse a target onto its prefix. Plain-`INSERT` UPSERT authority requires one balanced top-level `ON CONFLICT ... DO UPDATE SET` token sequence; ASCII whitespace and keyword case do not change it, while keyword-like string-literal content has no authority. Only ASCII whitespace is ignored at statement boundaries: non-ASCII space, separator, control, or BOM bytes outside a string literal remain syntax and fail before provider access, including after a semicolon. `INSERT OR REPLACE` and `REPLACE` reject every top-level conflict/action suffix rather than omitting its possible UPDATE effect. `DO NOTHING`, absent/duplicated/reordered/truncated compound actions, comments, CTEs, qualified or quoted targets, multiple statements, token/depth exhaustion, and unknown compounds fail before provider access. Both dry-run and live collect two authenticated, primary-served, mutation-free structured catalog observations, derive the reserved-relation graph from server-configured `CLOUDFLARE_MCP_D1_RESERVED_RELATIONS`, compose every effective primitive, and bind exact SQL bytes, params, target, identities, catalog snapshot, graph and decisions. Dry-run performs two reads, returns the exact composition digest, and exposes only read/composition steps with audit outcome `planned`. Live recomputes the digest under the permanent target guard and requires it. A guard change after those reads but before Prepared creation returns a complete blocked receipt with two provider calls, zero provider mutations, the complete live plan, and an explicit post-catalog/pre-DML phase proving that Prepared, DispatchReserved, and provider submission were not reached. Before any DML request live execution otherwise create-once installs canonical `Prepared` custody, atomically compares and exchanges exact incumbent bytes to `DispatchReserved`, synchronizes the target directory, and revalidates the held guard; the public live mutation plan marks both custody writes and the one provider submission as side effects. It also declares the required conditional post-provider custody compare-and-exchange: authenticated acknowledgement records `ProviderAssertionRecorded`, while a missing or ambiguous response records `Ambiguous`; exactly one successor is installed and both are side effects. Reached-phase evidence reports which successor was durably persisted and whether provider submission was attempted, while dry-run remains read-only and contains none of these custody or submission steps. If provider assertion/ambiguity derivation or that final compare-and-exchange fails after an attempted submission, the reconciliation-required envelope retains the exact aggregate response digest, byte size, provider lifecycle and safe outcome/error evidence, classifies derivation failure as `not_installed` and compare-and-exchange failure as `failed_or_unproven`, leaves successor installation unconfirmed, and permanently forbids replay; it never returns provider body text or raw identities. One bounded no-redirect provider request then retains exact complete response-body digest/size and dispatch/response/body/status lifecycle. The DML response envelope rejects duplicate or unknown top-level fields and requires exact `errors: []` and `messages: []`; any drift remains ambiguous. Lifecycle-owned monotonic accounting carries completed catalog calls through every later denial or custody failure and records a provider mutation only when the DML boundary reports an attempted dispatch, including response loss; shared guard errors and terminal classifications never reconstruct or reset these counts. Success metadata requires one clean primary-served result; `changed_db=false` with zero counters is a valid zero-change provider acknowledgement for every allowed statement kind. Provider acknowledgement is not terminal effect authority: it records an authenticated provider assertion and retains custody for the separately governed effect-specific readback/finalization path. Audit outcomes are closed to `planned`, `error`, and `reconciliation_required`; provider acknowledgement and response loss are `reconciliation_required`, never `success`. Transport loss, incomplete/malformed/contradictory response, storage drift, or exact replay records or preserves reconciliation-required custody and never redispatches the same attempt. Receipts expose only aggregate counts, booleans, hashes, phases, and decisions; raw account/database identities, opaque identities, SQL, params, relation names, paths, provider body text, and private catalog rows are not returned. Schema-changing SQL belongs in `d1_apply_migration_manifest`. |
...
Target-wide rename/delete have a distinct internal version-2 lifecycle product
in the same private sharded custody layout. It rederives the typed current
eight-step plan and consent from the canonical target, requested change, and
...
evidence fails closed. `Prepared` authorizes no provider access. Only the exact
owner-aware product may advance once to `DispatchReserved`; post-provider state
then advances once to aggregate-only `Acknowledged` or
`ReconciliationRequired`. Neither outcome finalizes, recovers, or authorizes
retry.
The outward receipt projects dispatch status separately from state observation
and causality. This boundary never observes current target state, always reports
causality as unproven, and always denies current-state and terminalization
authority. Exact replay returns only the incumbent custody with zero provider
calls and no state transition. Target-wide version-2 state rejects terminal
phases and any added terminal record as contradictory or outside its closed
schema.
The D1-local apply boundary also reconstructs the closed provider request from
that exact canonical product before acquiring the guard. No independent effect
selector reaches the reservation or client adapter. Fresh layout creation,
exact layout reuse, and ensure failure remain distinct aggregate evidence
products (`created`/one local mutation, `already_present`/zero, and
failed/unknown respectively), and all three retain
