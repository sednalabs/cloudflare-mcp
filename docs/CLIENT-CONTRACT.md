...
| `workers_capture_version_evidence` | `script_name`; `account_id` unless default account configured | `per_page`, `version_id`, `candidate_must_be_absent` | Read-only exact-evidence ceremony. It performs two complete stable version-list passes through a short terminal page, with a fixed page size from 1 through 100 and a 4,096-ID cap, optionally reads one exact version, and reads deployments twice. Version IDs, page/result digests, sanitized binding name/type descriptors, exact canonical script ETag, deployment projections, and authenticated request/response artifact SHA-256 values are returned; provider bodies and binding values are not. Duplicate IDs, drift, malformed pages/details/deployments, cross-target detail, missing ETag, or a candidate in any nonzero deployment fail closed. |
| `workers_upload_version` | `script_name`, exact `base_version_id` and `base_version_etag`, pinned `pre_upload_version_snapshot_sha256` and `pre_upload_deployment_snapshot_sha256`, `bindings_inherit="strict"`, closed `metadata` containing exactly `main_module`, `compatibility_date`, duplicate-free `compatibility_flags`, and the complete `bindings` array, plus exactly one reviewed module or multipart source; `account_id` unless default account configured | `main_module`, `content_type`, `per_page` (1–100), `dry_run`, `confirmation_token`, `reason` | Creates one disabled Worker version through exactly one `POST /accounts/{account}/workers/scripts/{script}/versions?bindings_inherit=strict`; it never creates a deployment. Arguments and metadata are deny-unknown, and every binding item is a tagged, deny-unknown supported-type schema before runtime canonicalization. Every other upload metadata field is rejected before dispatch, including exports reconciliation, migrations, assets/cache controls, annotations, dependencies, logpush/tails/tags/observability, placement, limits, and usage model. File-backed artifacts require a canonical relative path beneath the owner-only `CLOUDFLARE_MCP_WORKER_UPLOAD_ROOT`; every component is opened descriptor-relatively without following symlinks, and the final file is validated and read through that descriptor. Every requested inherited binding must name the exact base version, so implicit or `latest` inheritance is rejected. Provider detail that still contains `inherit` is also rejected because this bounded ceremony has no captured recursive chain. Explicit and provider-read binding objects otherwise pass the same closed per-type projection before request hashing or comparison: unknown types/fields fail closed, deprecated D1 `id` is normalized to `database_id`, and omitted AI Search instance `namespace` is normalized to its documented `default`. Dry-run makes no provider request and returns a confirmation token bound to the exact target, base, source/body/normalized-metadata/upload-contract digests, strict inheritance, and pre-state pins. Live apply additionally requires a canonical private shared `CLOUDFLARE_MCP_WORKER_VERSION_ATTEMPT_ROOT`. Before the provider POST, it durably creates append-only prepared and dispatched receipts under a deterministic authority key bound to the confirmation, target, base, upload contract and pre-state pins. The pinned provider state is captured again while the shared attempt guard is held, immediately before the prepared receipt can be consumed as dispatched, so a differently confirmed concurrent attempt cannot POST from stale pre-state. A complete response appends terminal custody; response loss or process death leaves the prior state permanent. Restored attempt namespaces are closed to at most the three named receipts. Each receipt is opened nonblocking and no-follow, validated from the held descriptor as one private bounded regular file before reading, read through a hard byte cap, then revalidated on that same descriptor. A FIFO, socket, device, dangling symlink, extra entry, oversized namespace, or other physically present malformed receipt is never equivalent to absence. Any present, concurrent, conflicting, or malformed attempt blocks before provider preflight and is reconciliation-only across restart, including a prepared attempt where no provider visibility can be proven. Apply then proves one new candidate ID/ETag, fully typed script/runtime/version-metadata projections, the complete binding projection expected from explicit metadata plus exact-base inheritance, and an unchanged sorted deployment projection including known `percentage` strategy with candidate absence. Success is `candidate_created_digest_only`; its source status is deliberately `source_provider_unverified` because Version Detail exposes a script ETag and runtime/resource projections but not the submitted module graph or source bytes. The result explicitly reports `exact_request_bytes_retained=false` and `canonical_request_manifest_retained=false`: only exact body, upload-contract, and authenticated-request digests plus size are returned, while durable attempt authority commits to the upload-contract digest. No response claims request-byte custody or provider-authenticated source identity. Request lifecycle evidence distinguishes preparation, attempted dispatch, and complete provider response. All upload response loss before or after provider visibility, malformed success/detail, provider rejection, runtime/binding mismatch, or post-readback failure is permanent no-retry evidence and must move to read-only reconciliation. Raw credential headers, response bodies, metadata values, binding values, module bytes, local custody paths, and confirmation tokens are never returned in error evidence; exact computed request/response SHA-256 digests and secret-safe attempt/projection hashes and names are. |
| `workers_reconcile_version_upload` | `script_name`, exact base ID/ETag, upload-contract SHA-256, pinned pre-upload version IDs and their SHA-256, pinned pre-upload deployment projection and SHA-256; `account_id` unless default account configured | `per_page` | Read-only response-loss evidence collection. It enumerates the current stable version set against the exact pinned predecessor set. Even when there is exactly one new disabled version, a still-present exact base, and unchanged deployments, provider inventory alone cannot prove that the lost request created that candidate or that its complete bytes and binding plan match the reviewed upload contract; the result remains `reconciliation_required` with `attribution_state=unattributed`. No or multiple candidates, a missing predecessor, supplied-hash mismatch, state drift, or a candidate in a deployment also fails closed. It never uploads or creates a deployment and never authorizes replay of the original POST. |
| `workers_upload_script` | `script_name`; `account_id` unless default account configured; exactly one of `script_path`, `script_content`, `script_content_base64`, or `multipart_path` | `main_module`, `metadata`, `content_type`, `create_only`, `dry_run`, `confirmation_token`, `reason` | Uploads a Worker module script or prebuilt multipart bundle through Cloudflare's Worker script endpoint. Path artifacts are opened once without following a final symlink, validated and size-bounded from the same descriptor, and then read from that descriptor. Dry-run prepares the upload and returns `required_confirmation_token` without calling Cloudflare; visible upload summaries include script/metadata SHA-256 digests and metadata keys, not raw metadata values. Apply requires that token, uploads the script or multipart bundle, then reads back Worker settings and returns `readback_verification`; a different non-empty `main_module` fails closed. For create-only module uploads, a null settings `main_module` is resolved only by exhaustive, stable, etag-bound listing/version-detail evidence; malformed, incomplete, ambiguous, or conflicting evidence fails closed. When `create_only` is true, the apply request sends Cloudflare's atomic `If-None-Match: *` precondition; an existing script returns `workers.upload_create_only_conflict` without overwrite or retry. Timeout, transport, response-read/decoding, retryable 5xx, and success envelopes with a missing or null result return `workers.upload_create_only_outcome_uncertain` with `retryable:false`; read back the Worker and reconcile provider evidence before retrying or claiming creation. The default elicitation configuration treats this as an action-time approval tool. |
...
- Broad cache actions add local confirmation: `cache_purge` with `payload.everything=true` and `cache_rules` with `action=replace_all` require echoing the token returned by dry-run. `workers_upload_script` and `workers_upload_version` also require echoing the dry-run token because they submit executable Worker code. `workers_upload_version` additionally requires exact stable pre-state pins, strict exact-base binding inheritance, and proves that the new candidate is not deployed. `waf_ruleset_apply_change` requires the token returned by `waf_ruleset_plan_change` because WAF action, ordering, list, and cap mistakes can affect production traffic.
