...
| `api_mutate` | `operation_id` | `path_params`, `query`, `body`, `dry_run`, `confirmation_token`, `reason` | Executes catalog `POST`/`PUT`/`PATCH`/`DELETE` operations through dry-run confirmation; high-risk and denied-by-default operations fail closed. The generic `worker-script-put-content` upload is denied; use `workers_upload_script` for its digest-bound upload, confirmation, and readback contract. Valid escaped JSON-string `body` values are normalized into real JSON and reported with `body_normalized_from_json_string`. |
...
| `workers_upload_script` | `script_name`; `account_id` unless default account configured; exactly one of `script_path`, `script_content`, `script_content_base64`, or `multipart_path` | `main_module`, `metadata`, `content_type`, `create_only`, `dry_run`, `confirmation_token`, `reason` | Uploads a Worker module script or prebuilt multipart bundle. Existing-worker dry-run reads settings, bindings, and schedules, returns a redacted preservation manifest, and binds only its nonsecret projection digest into the confirmation token. Apply rereads current state before mutation, uses Cloudflare's content-only endpoint, and succeeds only when upload identity plus an internal exact full-state post-apply comparison match. Secret-bearing settings digests, secret values, and resource identifiers are never returned or incorporated into the confirmation token; missing, malformed, or ambiguous preservation evidence fails closed before mutation. An omitted settings `main_module` is accepted only when this complete content-only proof passes. Create-only upload keeps the atomic `If-None-Match: *` and exhaustive version-attestation contract. The default elicitation configuration treats this as an action-time approval tool. |
...
names/types, cron strings, setting keys, counts, and the SHA-256 digest of that
redacted nonsecret projection; binding values, resource identifiers, secret
values, and their digests remain inside the adapter. The confirmation token
therefore binds the upload to the redacted preservation shape, not to
secret-bearing values. Apply repeats the reads immediately before mutation and
sends code through Cloudflare's `/content` endpoint. Post-apply settings,
bindings, and schedules must match the exact internal pre-apply state, and the
upload response must identify the exact script with a nonblank etag. Transport-only module
...
remain protected by an exact internal pre/post comparison, while every other
supplied metadata key must match the current setting exactly; use
