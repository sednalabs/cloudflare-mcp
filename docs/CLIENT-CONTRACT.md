...
| `d1_rename_database` | `database_id`, `name`; `account_id` unless default account configured | `dry_run`, `confirmation_token`, `reason` | Curated D1 rename planner with a closed input schema. Dry-run returns the complete static six-step plan, explicit versioned consent binding, and `required_confirmation_token`. The token binds the normalized target, rename, reason, operation/version, plan digest, and full plan. Within this governed MCP tool, missing or stale live consent fails before guard/provider access; exact consent currently returns `durable_reservation_not_installed` with zero provider calls because live execution remains disabled pending durable reservation/recovery custody. RMCP text and structured JSON are whole-payload equivalent after final audit/evidence insertion. Direct callers of the public Cloudflare client adapter require their own governed wrapper before enabling target-wide mutation. |
| `d1_delete_database` | `database_id`; `account_id` unless default account configured | `dry_run`, `confirmation_token`, `reason` | Curated high-risk D1 delete planner with the same closed input schema and complete six-step plan-bound consent contract. Target, operation/version, delete intent, reason, plan digest, and full plan changes invalidate the token. Within this governed MCP tool, missing or stale live consent fails before guard/provider access; exact consent currently returns `durable_reservation_not_installed` with zero provider calls because live execution remains disabled pending durable reservation/recovery custody. RMCP text and structured JSON are whole-payload equivalent after final audit/evidence insertion. Direct callers of the public Cloudflare client adapter require their own governed wrapper before enabling target-wide mutation. |
