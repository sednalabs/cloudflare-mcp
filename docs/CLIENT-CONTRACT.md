...
| `d1_reconcile_migration_manifest` | `database_id`, `migration_family`, complete exact-byte `manifest[]`, `approved_plan_sha256`, `lease_nonce`, `lease_payload_sha256`, ordered bounded `state_expectations[]`; `account_id` unless default account configured | `migrations_table`, `effect_assertion_id` | Read-only recovery boundary for exactly one retained `active.lease.json` or `retiring.lease.json`. It locks the existing permanent guard, binds exact canonical target/family/plan/nonce/payload identity, and never creates, renames, rewrites, retires, or unlinks custody evidence. The caller cannot supply SQL. The tool builds one bounded multi-statement SELECT batch over the exact ledger, declared `sqlite_master` names, table-valued `table_xinfo`, `foreign_key_list`, and bounded `foreign_key_check`, executes that complete batch exactly twice without adapter retries, and requires canonical-equivalent snapshots. `effect_assertion_id=schema_create_only_v1` is registry-backed and accepts only exact CREATE TABLE/INDEX manifest SQL; arbitrary DML, ALTER, DROP, PRAGMA, trigger, view, data-copy, missing assertion, or unclassified SQL returns `effect_proof_unavailable`. Caller claims such as `schema_only` are not authority. Successful evidence classifies `not_committed`, `partial_state_converged`, or `full_state_converged` as documented atomic-state inference, never provider-attempt causality. Every result says `retry_decision=do_not_retry_same_attempt`, `lease_decision=retain`, and reports query/body/snapshot digests plus a future-transition `reconciliation_plan_sha256`. Capability gaps, unavailable provider/auth/transport evidence, malformed or unstable results, missing/unexpected schema, SQL-digest/xinfo/FK mismatch, and any FK violation fail closed. This slice cannot retire a lease or persist a terminal reconciliation receipt. |
